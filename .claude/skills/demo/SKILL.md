...
- If it's missing, have them run **in their own terminal** (not through you —
  ...
  they confirm it completed. (No need to keep re-checking by hand — Step 6's
  `/rig-doctor` confirms the scope and everything else once rig is installed.)
...
## Step 6 — Verify with rig-doctor

Before building, run the health check — it re-checks the whole setup at once (gh
auth + the `project` scope, the board is reachable, the config is valid, labels
exist, `.rig/epics` gitignored, CI gates) and applies the safe fixes:

```
/rig-doctor --fix
```

Clear anything it flags — it prints the exact command for the manual ones (e.g.
`gh auth refresh -s project` if the scope is still missing). Re-run until it's
green, then continue.

## Step 7 — Build the foundations (Milestone 0)
...
## Step 8 — Kick off the epic, then walk away
